...
def train_recognizer(face_imgs, face_labels, userID_model):
    ...
    try: 
        face_recognizer.train(face_imgs, np.array(face_labels))
    except cv2.error:
        logging.error(e)
        print(e)
        return None
    face_recognizer.write(userID_model)

    count = 0
    for img in face_imgs:
        label, confidence = face_recognizer.predict(img)
        print("precdicted = {} with confidence = {}  actual = {}".format(label,confidence, face_labels[count]))
        count+=1 
...
def import_face_recognizer(userID_model = 'model/face_recognizer.yml' ): 
    face_recognizer = cv2.face.LBPHFaceRecognizer_create()
    face_recognizer.read(userID_model)
    return face_recognizer
...
def main():
    ...
    userID_model = 'model/face_recognizer.yml' 
    ...
    if var1 == '-capture':
        ...
        face_recognizer = train_recognizer(face_imgs, face_labels, userID_model)
